...
export default function OfferCard({ oferta }: { oferta: Oferta }) {
  ...
  return (
    <Link
      ...
      className="flex flex-col overflow-hidden rounded-xl2 bg-white ring-1 ring-ink/10"
    >
      <div className="relative aspect-square bg-ink/5">
        ...
        {desconto && (
          <span className="absolute left-2 top-2 rounded-full bg-discount px-2 py-1 text-xs font-semibold text-[#5B4300]">
            -{desconto}%
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <span className="text-xs font-medium text-ink/50">{oferta.loja}</span>
